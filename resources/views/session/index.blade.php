        ...
        <div class="ibox-body">
            <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Wakut Per Sesi</th>
                        <th>Minimum Sesi</th>
                        <!-- <th>Harga</th> -->
                        <th>Diskon (Persentase)</th>
                        <th>Diskon Pada Sesi Ke</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        <td>{{ $loop->index + 1 }}</td>
                        <td>{{ $value->waktu_per_sesi }}</td>
                        <td>{{ $value->minimum_sesi }}</td>
                        <!-- <td>{{ $value->harga }}</td> -->
                        <td>{{ $value->discount * 100 }}%</td>
                        <td>{{ $value->discount_sesi_ke }}</td>
                        <td>
                            <form action="/sessions/{{ $value->id }}/delete" method="post">
                                @csrf
...
<!-- MODAL -->
@include('session._edit')
@endsection


@section('extra_javascript')
<script src="{{ asset('assets/js/session/session.js') }}" type="text/javascript"> </script>
@endsection