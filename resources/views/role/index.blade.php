            ...
            <table class="table table-striped table-bordered table-hover" id="example-table" cellspacing="0" width="100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $value->nama }}</td>
                        <td>
                            <form action="/roles/{{ $value->id }}/delete" method="post">
                                @csrf
                                <button data-id="{{ $value->id }}" type="button" class="edit btn btn-success">Edit</button>
                                <button data-id="{{ $value->id }}" type="button" class="setting_akses btn btn-primary">Atur Akses</button>
                                @if(!$value->is_default)
...
@include('role._add')
@include('role._edit')
@include('role._setting_akses')
@endsection


@section('extra_javascript')
<script src="{{ asset('assets/js/role/role.js') }}" type="text/javascript"> </script>
@endsection